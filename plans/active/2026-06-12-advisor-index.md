...
- A 2026-06-12 advisor run at `f8e28be91` covered the seams the Jun 9 audit
  ...
  diarization). **That run's branch never merged**, so its plans were stranded
  off-main — see "Reconciliation" below.
...
## Reconciliation (prior stranded advisor run)

The 2026-06-12 advisor run at `f8e28be91` produced two artifacts that never
reached main because its branch wasn't merged:

- **`2026-06-onboarding-stall-watchdog-test.md`** — a P1 test plan. **Recovered
  into this run** (re-validated against `3f9361005`; line numbers refreshed). It
  is the third plan in the table above.
- **The prior `2026-06-advisor-index.md`** — superseded by this index. Its
  AUDIT-074/075/076/080 *fixes* did land on main via separate commits, so only
  the index doc itself was lost; this index carries its still-relevant
  opportunistic items and rejections forward below.
