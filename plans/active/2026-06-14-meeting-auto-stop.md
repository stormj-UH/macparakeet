...
Native apps detectable by bundle ID via `NSWorkspace.shared.runningApplications`: Zoom `us.zoom.xos`, Teams `com.microsoft.teams2`/`com.microsoft.teams`, Webex `com.cisco.webexmeetingsapp`/`Cisco-Systems.Spark`, FaceTime `com.apple.FaceTime`. **Known limitation (document in Settings help):** browser-based meetings (a Meet/Teams tab) and in-person recordings have no app to quit — they are covered by the Phase B silence signal.
